...
- ➡️ **Reviewer Finding Reports** — After each iteration, the Code Reviewer, QA Engineer, and Security Engineer each write a structured Markdown report of their findings (grouped by severity: critical, major, minor, info) to a persistent artifacts directory (e.g. `docs/reviews/iteration-N-reviewer.md`). Reports accumulate across iterations so the developer, PO, and human can trace how issues were found and resolved. `Impact: High`
  - `npm run start -- self-improve "After each development loop iteration, save the findings from Code Reviewer, QA Engineer, and Security Engineer as structured Markdown reports to a docs/reviews/ directory in the workspace. File naming: iteration-N-reviewer.md, iteration-N-qa.md, iteration-N-security.md. Each report should contain: agent name, iteration number, score, and issues grouped by severity (critical, major, minor, info) with descriptions and suggestions. The orchestrator should write these files after each evaluator completes. Reports accumulate across iterations."`
...
- ➡️ **Parallel Agent Execution** — Run independent agents (Reviewer, QA, Security) in parallel during the development loop instead of sequentially, reducing wall-clock time. `Impact: High`
